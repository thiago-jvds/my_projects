...
def sequence_prob(seq, trans_model, obs_model, prior, observations):
    prob = 0.0
    prob += np.log(prior[seq[0]])
    # print(seq)
    for i in range(len(seq)):
        if observations[i] is not None:
            prob += np.log(obs_model(seq[i])[observations[i]])
        if i != len(seq) - 1:
            prob += np.log(trans_model(seq[i])[seq[i+1]])
    return prob
#-----------------------------------------------------------------------------
