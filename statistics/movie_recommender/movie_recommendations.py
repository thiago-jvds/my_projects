...
def compute_posterior(prior, likelihood, y):
    ...
    for m in range(M):
        ...
        log_pYgX = np.sum([log_likelihood[y[n], m] for n in range(N)])
            
        log_answer[m] = log_prior[m] + log_pYgX
        
        if m == 0: marginal = log_answer[0]
        else:      marginal = np.logaddexp(marginal, log_answer[m])
        
    #
    # END OF YOUR CODE FOR PART (b)
    # -------------------------------------------------------------------------

    return np.exp(log_answer - marginal)
...
def compute_movie_rating_likelihood(M):
    ...
    for m in range(M):
        ...
        alpha = 1/np.sum(likelihood[:, m], axis=0)
        
        likelihood[:,m] = likelihood[:,m]*alpha
    #
...
def compute_entropy(distribution):
    ...
    for j in range(M):
        ...
        if distribution[j] <= 1e-8:
            continue
        
        else:
            entropy += distribution[j]*np.log2(distribution[j])
    #
...
def compute_true_movie_rating_posterior_entropies(num_observations):
    ...
    for i, movie_id in enumerate(movie_id_list):
        ...
        posterior_entropies[i] = compute_entropy(posterior[i])
...
def main():
    ...
    y = [0]*2 + [1]*1
    print("My answer:")
    print(a:=compute_posterior(prior, likelihood, y))
    # tot = np.sum(np.exp(a))
    # print(np.exp(a)/tot)
    print()
    ...
    _, MAP_ratings = infer_true_movie_ratings()
    
    top_10 = np.argpartition(MAP_ratings, -10)[-10:]
    
    for movie in range(10):
        print(f"Top {movie+1}: {movie_data_helper.get_movie_name(top_10[movie])}")
        print()
        
    print('-----')
    
    worst_10 = np.argpartition(MAP_ratings, 10)[:10]
    
    for movie in range(10):
        print(f"Worst {movie+1}: {movie_data_helper.get_movie_name(worst_10[movie])}")
        print()
        
    plt.plot([i for i in range(1,201)], 
             [np.mean(compute_true_movie_rating_posterior_entropies(i)) for i in range(1,201)])
